"use strict";

var express = require('express'); // Correct import statement with proper destructuring


var _require = require('../controllers/authController'),
    registerUser = _require.registerUser,
    loginUser = _require.loginUser,
    logoutUser = _require.logoutUser,
    forgotPassword = _require.forgotPassword,
    resetPassword = _require.resetPassword,
    getUserProfile = _require.getUserProfile; // Ensure the middleware is exported correctly in authenticate.js


var _require2 = require("../middleware/authenticate"),
    isAuthenticatedUser = _require2.isAuthenticatedUser;

var router = express.Router();
router.route('/register').post(registerUser);
router.route('/login').post(loginUser);
router.route('/logout').get(logoutUser);
router.route('/password/forgot').post(forgotPassword);
router.route('/password/reset/:token').post(resetPassword);
router.route('/myprofile').get(isAuthenticatedUser, getUserProfile); // Both must be imported correctly

module.exports = router;